...
namespace InkaArt.Business.Algorithm
{
    class IndexController
    {
        ...
        //////////TEMPORAL11111111111111111111111

        public List<Temp_GraspInput> GetCandidates(List<Worker> workers, JobController jobs)
        {
            List<Temp_GraspInput> candidates = new List<Temp_GraspInput>();
            foreach (Index index in indexes)
            {
                Worker selected_worker = null;
                foreach (Worker worker in workers)
                {
                    if (worker.ID == index.Worker) { selected_worker = worker; break; }
                }
                Job selected_job = null;
                for (int i = 0; i < jobs.Count(); i++)
                {
                    if (jobs[i].ID == index.Job) { selected_job = jobs[i]; break; }
                }
                candidates.Add(new Temp_GraspInput(selected_worker, selected_job, 0.5*index.BreakageIndex + 0.5*index.TimeIndex));
            }
            return candidates;
        }
    }
