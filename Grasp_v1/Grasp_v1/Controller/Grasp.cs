...
namespace InkaArt.Controller
{
    class Grasp
    {
        ...
        public Grasp(WorkerManager workers, ProcessManager processes, ProductManager products,
            ProcessProductManager processes_products, RatioManager ratios)
        {
        ...
        public List<int[]> GraspAlgorithm()
        {
            ...
            Console.WriteLine("TIEMPO TOTAL DE EJECUCIÓN: {0}.{1} segundos",
        ...
        private Solution GenerateSolution(List<Assignment> candidates)
        {
            ...
            List<ProcessProduct> current_product = new List<ProcessProduct>();
                ...
                construction_iteration++)
            {
                ...
                if (rcl.Count <= 0)
                {
                    //Si la lista está vacía, no hay problema, pero si la lista no
                    //está vacía, borrar las últimas agregaciones
